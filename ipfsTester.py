...
#Configuration for ipfs installation
client = ipfshttpclient.connect()

#Configuration for filesystem logging.
records_file = "./records_ipfs.json"
save_to_file = True
...
def stopwatch(func):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            events.request_failure.fire(request_type="IPFS", #request_type: The "family" of the task.
            ...
            events.request_success.fire(request_type="IPFS", #request_type: The "family" of the task.
                                        name=task_name, #name: The name of the task.
                                        response_time=total, #response_time: The total time elapsed for this task.
...
class ACESClient:
        ...
        for file in user.test_files:
            try:
                ...
                with open(file, 'rb') as data:
                    ...
                    #List that will be added as JSON
                    image_object = [ user.bucket_id,
                            file.split('/')[-1],
                            str(data.read()),
                    ...
                    #add to ipfs
                    hashof_obj = client.add_json(image_object)
                    user.hashof_test_files.append(hashof_obj)
    ...
    def read_small_files_aces(self,user):
        ...
        #After reading a test file we write the object to the db
        #and we keep the hash of it in a list so each testfile 
        #corresponds to a hash
        # WARNING: ORDER OF THE LISTS SHOULD NOT BE CHANGED FOR THE SAKE OF ACCURACY
        for file, hashof_obj in zip(user.test_files, user.hashof_test_files):
            try:
                response = None
                try:
                    startClock = datetime.datetime.now()
                    ...
                    #tmp = file.split('/')[-1]
                    response = client.get_json(hashof_obj)
...
class ACESUser(User):
    ...
    def __init__(self,env):
        ...
        #Hash is required to get the file
        self.hashof_test_files = []
